...
from cad_algo.boolean.parser import Parser
from cad_algo.boolean.sop import BaseSOP

parser = Parser(6, tuple("abcdef"))


def parse_sop(name: str, expr: str, expected: str) -> BaseSOP:
    """Parse a SOP boolean expression and print the results."""
    result = parser.parse(expr)
    print_test(name, expr, result, expected)
    if not isinstance(result, BaseSOP):
        msg = f"Result for test '{name}' is not a SOP"
        raise TypeError(msg)
    return result


def print_test(name: str, input_str: str, result: BaseSOP, expected: str) -> None:
...
def testsop() -> None:  # noqa: PLR0915
    ...
    c1 = parse_sop("Basic cube (c1)", "a*b*c", "abc")
    c2 = parse_sop("Cube with complement (c2)", "b*c*d*~e", "bcd~e")
    c3 = parse_sop("Another cube (c3)", "c*d*e*~f", "cde~f")
    c4 = parse_sop("Yet another cube (c4)", "a*c*~d", "ac~d")
    c5 = parse_sop("And another (c5)", "c*d*~e", "cd~e")

    c_zero = parse_sop("Special zero cube", "0", "0")
    c_one = parse_sop("Special one cube", "1", "1")

    print("\n***** Testing SOP Expressions *****")

    parse_sop("Basic sum", "a + b + c", "a + b + c")
    parse_sop("Apply De Morgan's", "~(a*~b*d*~e)", "b + e + ~a + ~d")
    parse_sop("Complex expr", "a*b*~c + ~b + d*~e + a*f", "ab~c + af + d~e + ~b")
    ...
    exp3 = parse_sop(
        "Nested parentheses (exp3)", "((a*b)) + ~(b*c) + b*c*d", "ab + bcd + ~b + ~c"
    )
    print("\t^minimized wrt SCC but not complete")
    exp4 = parse_sop(
        "Another SOP (exp4)", "a*b + ~b + ~c + b*c*d", "ab + bcd + ~b + ~c"
    )
    print("\t^minimized wrt SCC but not complete")
    exp5 = parse_sop(
        "Big SOP (exp5)",
        "~b*~c*~d + a*d + a*c*~d + ~a*c*d + ~a*~b*c + ~a*~c*~d + ~a*~b*~c",
        "ac~d + ad + ~acd + ~a~bc + ~a~b~c + ~a~c~d + ~b~c~d",
    )
    print("\t^minimized wrt SCC but not complete")
    ...
    print_test("Invert SOP", f"~({exp6})", ~exp6, "~a + ~bd + ~c")

    print("\n***** Testing Complete SOP *****")

    print_test(
        "Complete exp1", f"{exp1}", exp1.complete(), "ac + a~b + bc + ~ab + ~a~c + ~b~c"
    )
    print(
    ...
    print_test("Complete exp3", f"{exp3}", exp3.complete(), "a + d + ~b + ~c")
    print_test("Complete exp4", f"{exp4}", exp4.complete(), "a + d + ~b + ~c")
    print_test(
        "Complete exp5", f"{exp5}", exp5.complete(), "ac + ad + cd + ~a~c~d + ~b"
    )
    ...
    print_bool("isTautology", "1", result=c_one.is_tautology(), expected=True)
    print_bool("isTautology", "0", result=c_zero.is_tautology(), expected=False)
    print_bool("isTautology", str(exp1), result=exp1.is_tautology(), expected=False)
    print_bool("isTautology", str(taut1), result=taut1.is_tautology(), expected=True)

    expr = parser.parse("a*~c + b*c + ~a*~d + a*~b*~c + b*~d")
    print_bool("isTautology", str(expr), result=expr.is_tautology(), expected=False)
    expr = parser.parse("b*c + ~b*c*d + b*~c")
    print_bool("isTautology", str(expr), result=expr.is_tautology(), expected=False)
    expr = parser.parse("a*b*c + a*c*~d + ~a*~b*~c + ~a*d + b*~c + ~a*c")
    print_bool("isTautology", str(expr), result=expr.is_tautology(), expected=False)
    expr = parser.parse(
        "~a*~b*~c*d+~a*~c*~d+~a*b*~c+a*~c+a*~b*c+a*b*c*~d+b*d+~a*c*~d+~a*c*d"
    )
    print_bool("isTautology", str(expr), result=expr.is_tautology(), expected=True)
